...
@Component({
  selector: 'app-individual-profile-update',
  templateUrl: './individual-profile-update.component.html',
  styleUrls: ['./individual-profile-update.component.css']
})
export class IndividualProfileUpdateComponent implements OnInit {
  ...
  updateApplication() {
    ...
    let correction;
    this.changeArray.forEach(x => {
      if (correction) {
        correction = correction + ', ' + x.sec + ' changed from ' + x.oldValue + ' to ' + x.newValue;
      } else {
        correction = x.sec + ' changed from ' + x.oldValue + ' to ' + x.newValue
      }
    });
    console.log('String : ' + correction);
    if (this.manageAdmitCardsForm.valid) {
      const reqObj = {
        fullName: this.manageAdmitCardsForm.controls['name'].value,
        fatherName: this.manageAdmitCardsForm.controls['fatherName'].value,
        dob: this.manageAdmitCardsForm.controls['dob'].value,
        gender: this.manageAdmitCardsForm.controls['gender'].value,
        categoryId: this.manageAdmitCardsForm.controls['category'].value,
        subCategoryId: this.manageAdmitCardsForm.controls['subCategory'].value,
        applicationId: this.response.applicationId,
        isGorkha: this.manageAdmitCardsForm.controls['isGorkha'].value,
        remarks: this.manageAdmitCardsForm.controls['remarks'].value,
        hpBonafideCertifcateNo: this.manageAdmitCardsForm.controls['hpBonafideCertifcateNo'].value,
        postAppliedFor: this.manageAdmitCardsForm.controls['postAppliedFor'].value,
        correction: correction
      };
      this.applicationService.updateApplication(reqObj).then((resp: any) => {
        if (resp.status == 200) {
          this.submitApplicationForm = false;
          this.enableForm = false;
          this.initializeForm();
