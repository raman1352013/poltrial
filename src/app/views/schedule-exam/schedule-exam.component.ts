...
export class ScheduleExamComponent implements OnInit {
  ...
  checkRole() {
    this.sharedService.getLocalStorageItem('userType').then((resp: any) => {
      if (resp) {
        if (resp.id !== 2 && resp.id !== 3 && resp.id !== 14 && resp.id !== 15) {
  ...
  getApplicationSubmitted(application) {
    this.enableScanner = false;
    this.applicationService.getApplicationExam(application).then((resp: any) => {
      if (resp.status == 200) {
        if (resp['body']['responseCode'] == 200) {
          ...
          this.manageAdmitCardsForm.controls['applicationNo'].setValue(response.rollNo);
          ...
          this.manageAdmitCardsForm.controls['isGorkha'].setValue(response.examVenue);
  ...
  register() {
    this.sharedService.confirm2('Confirm', 'Do you want to register?').then((res: any) => {
      console.log('RESP: ', res)
      if (res == true) {
        const reqObj = {
          action: 1,
          rollNo: this.manageAdmitCardsForm.controls['applicationNo'].value
        }
        this.applicationService.updateExam(reqObj).then((resp: any) => {
          if(resp.status == 200) {
            if (resp['body']['responseCode'] == 200) {
              this.broadJumpFormSubmit = false;
              this.initializeForm();
  ...
  downloadAdmitCard() {
    this.jsService.examAdmitCardPdf(this.response.rollNo).then((resp: any) => {
      if (resp) {
