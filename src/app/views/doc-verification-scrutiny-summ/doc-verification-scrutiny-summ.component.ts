...
@Component({
  selector: 'app-doc-verification-scrutiny-summ',
  templateUrl: './doc-verification-scrutiny-summ.component.html',
  styleUrls: ['./doc-verification-scrutiny-summ.component.css']
})
export class DocVerificationScrutinySummComponent implements OnInit {
  ...
  petSummary(reqObj) {
    ...
    this.reportsService.scrutinySummary(reqObj).then((resp: any) => {
      if (resp.status == 200) {
        if (resp['body']['responseCode'] == 200) {
          // this.initializeForm();
          this.summaryList = resp['body']['responseObject'].scrutinySummary;
