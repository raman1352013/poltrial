...
export class AppealComponent implements OnInit {
  ...
  getApplicationSubmitted(application) {
    ...
    this.driverFormSubmit = false;

    this.heightForm.reset();
    this.chestForm.reset();
    this.raceForm.reset();
    this.heighJumpForm.reset();
    this.broadJumpForm.reset();
    ...
    this.applicationService.petResult(application).then((resp: any) => {
      if (resp.status == 200) {
        if (resp['body']['responseCode'] == 200) {
          ...
          this.heightMeasure = {
            heightFeetQa: response.heightMinimum.heightFeetQa,
            heightInchesQa: response.heightMinimum.heightInchesQa
          };
  ...
  getFeet() {
    ...
    let score;
    console.log('this.feet', this.feet)
    console.log('this.inches', this.inches)
  ...
  getInchesA() {
    if (this.chestForm.controls['chestA'].value && this.chestForm.controls['chestB'].value) {
      if (this.response && this.response.chestMinimum) {
        if (this.chestForm.controls['chestA'].value >= this.response.chestMinimum.chestQa && this.chestForm.controls['chestB'].value >= this.response.chestMinimum.chestQb) {
  ...
  getInchesB() {
    if (this.chestForm.controls['chestA'].value && this.chestForm.controls['chestB'].value) {
      if (this.response && this.response && this.response.chestMinimum) {
        if (this.chestForm.controls['chestA'].value >= this.response.chestMinimum.chestQa && this.chestForm.controls['chestB'].value >= this.response.chestMinimum.chestQb) {
          this.chestForm.controls['result'].setValue('Qualified');
          // this.status = 'Qualified';
        } else {
  ...
  openmd(content) {
    ...
    this.modalService.open(content, {
      ...
      if (this.heightButton == false) {
        ...
        // if (this.usedFor == '1') {
        this.markStatus(this.usedFor);
        // }
      } else if (this.driverEdit == false) {
        this.updateDriver()
      }
  ...
  updateDriver() {
    const reqObj = {
      ...
      petDetails: {
        petDriver: {
          driverExamStatus: this.driverStatus == 'Qualified' ? 'P' : 'F',
          driverExamMarks: this.driverForm.controls['marks'].value,
          remarks: this.driverForm.controls['remarks'].value
        }
      }
    };
    console.log('reqObj ', reqObj)
  ...
  getMark() {
    if (this.driverForm.controls['marks'].value >= 22) {
      this.driverStatus = 'Qualified';
    } else {
      this.driverStatus = 'Not Qualified';
    }
