...
@Component({
  selector: 'app-doc-verification-lmv',
  templateUrl: './doc-verification-lmv.component.html',
  styleUrls: ['./doc-verification-lmv.component.css']
})
export class DocVerificationLmvComponent implements OnInit {
  ...
  getApplicationSubmitted(application) {
    this.enableScanner = false;
    this.applicationService.getApplicationScrutiny(application, 4).then((resp: any) => {
      if (resp.status == 200) {
  ...
  hasLmvLic() {
    const license = this.generateAdmitCardForm.controls['license'].value
    console.log('backward', license);
    this.generateAdmitCardForm.controls['marksScored'].setValue(license)
  }
  ...
  submitApplication() {
    ...
    const reqObj = {
      ...
      isLmv: isLmv,
      lmvEvaluationMarks: this.generateAdmitCardForm.controls['marksScored'].value,
      mode: 'SCR',
      lmvRemarks:  this.generateAdmitCardForm.controls['remarks'].value,
    }
