...
@Component({
  selector: 'app-daily-summary-scrutiny-land',
  templateUrl: './daily-summary-scrutiny-land.component.html',
  styleUrls: ['./daily-summary-scrutiny-land.component.css']
})
export class DailySummaryScrutinyLandComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // this.checkRole();
    // this.getVenue();
    this.scrutinyIndividualReports()
  }

  scrutinyIndividualReports() {
    this.reportsService.scrutinyIndividualReports().then((resp: any) => {
      if (resp.status == 200) {
        if (resp['body']['responseCode'] == 200) {
          console.log('ReG Report  ' , resp['body']['responseObject'])
          this.summaryList = resp['body']['responseObject']
        }
      }
    })
  }
