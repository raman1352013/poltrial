...
@Component({
  selector: 'app-doc-verification-edu-qualification',
  templateUrl: './doc-verification-edu-qualification.component.html',
  styleUrls: ['./doc-verification-edu-qualification.component.css']
})
export class DocVerificationEduQualificationComponent implements OnInit {
  ...
  getApplicationSubmitted(application) {
    this.enableScanner = false;
    this.applicationService.getApplicationScrutiny(application, 2).then((resp: any) => {
      if (resp.status == 200) {
  ...
  marksScored(event) {
    ...
    const maximumMarks = Number(this.generateAdmitCardForm.controls['maximumMarks'].value);
    ...
    if(maximumMarks && marksScored) {
      if(maximumMarks > marksScored || maximumMarks == marksScored) {  
        ...
        this.generateAdmitCardForm.controls['eduMarksInPercentage'].setValue(percentage.toFixed(3));
        let calMarks;
        ...
        this.generateAdmitCardForm.controls['eduEvaluationMarks'].setValue(calMarks.toFixed(3))
        console.log('percentage ' , percentage)
        console.log('calculatedMarks ' , calculatedMarks)
      } else {
        console.log('marksScored ' )
        this.generateAdmitCardForm.controls['eduEvaluationMarks'].setValue('')
        this.generateAdmitCardForm.controls['marksScored'].setValue('')
        this.generateAdmitCardForm.controls['eduMarksInPercentage'].setValue('');
        this.generateAdmitCardForm.controls['eduEvaluationMarks'].setValue('')
  ...
  submitApplication() {
    ...
    let reqObj = {
      ...
      eduMaxMarks: this.generateAdmitCardForm.controls['maximumMarks'].value,
      eduMarksScore: this.generateAdmitCardForm.controls['marksScored'].value,
      eduMarksInPercentage: this.generateAdmitCardForm.controls['eduMarksInPercentage'].value,
      eduEvaluationMarks: this.generateAdmitCardForm.controls['eduEvaluationMarks'].value,
      mode: 'SCR'
    }
    console.log('Reqobj ', reqObj)
