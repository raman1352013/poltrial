...
@Component({
  selector: 'app-doc-verification-individual-result',
  templateUrl: './doc-verification-individual-result.component.html',
  styleUrls: ['./doc-verification-individual-result.component.css']
})
export class DocVerificationIndividualResultComponent implements OnInit {
  ...
  resultObj = {
    ...
    backwardAreaLandless: null,
    lMVLicense: null,
    nssNccBharatScout: null,
    certificateInSports: null,
    medalWinnerofInternationalOlympicNationalLevelSports: null,
    landlessEvaluationMarks: null,
    eduMarksInPercentage: null,
    eduMarksScore: null,
    eduMaxMarks: null
  }
  constructor(
    ...
    this.applicationService.getApplicationScrutiny(application, 6).then((resp: any) => {
      if (resp.status == 200) {
        if (resp['body']['responseCode'] == 200) {
          ...
          if(response.scrutinyDetails) {
            let result = response.scrutinyDetails;
            // this.resultObj = {
            //   registration: null,
            //   educationalQualification: null,
            //   backwardAreaLandless: null,
            ...
            if(result.registeredBy) {
              this.resultObj.registration = 'Done'
            }
            this.resultObj.educationalQualification = result.eduEvaluationMarks;
            this.resultObj.eduMarksInPercentage = result.eduMarksInPercentage;
            this.resultObj.eduMarksScore = result.eduMarksScore;

            this.resultObj.backwardAreaLandless = result.bkdaEvaluationMarks

            this.resultObj.landlessEvaluationMarks = result.landlessEvaluationMarks;

            this.resultObj.lMVLicense = result.lmvEvaluationMarks;

            this.resultObj.nssNccBharatScout = result.nssNccEvaluationMarks;

            this.resultObj.certificateInSports = result.sportsEvaluationMarks;
            this.resultObj.eduMaxMarks = result.eduMaxMarks;

            this.resultObj.medalWinnerofInternationalOlympicNationalLevelSports = result.mdwEvaluationMarks;
          }
        } else {
  ...
  downloadAdmitCard() {
    this.jsService.scrutinyAdmitCardPdf(this.searchForm.controls['applicationNumber'].value).then((resp: any) => {
      if (resp) {
        // this.blob = new Blob([resp], { type: 'application/pdf' });
        const downloadURL = window.URL.createObjectURL(resp);
        const link = document.createElement('a');
        link.href = downloadURL;
        link.download = "Admit card-Scrutiny-"+this.searchForm.controls['applicationNumber'].value +".pdf";
        link.click();
