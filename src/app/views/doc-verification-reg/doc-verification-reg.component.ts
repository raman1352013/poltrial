...
import { JsService } from 'src/app/services/js/js.service';

@Component({
  selector: 'app-doc-verification-reg',
  templateUrl: './doc-verification-reg.component.html',
  styleUrls: ['./doc-verification-reg.component.css']
})
export class DocVerificationRegComponent implements OnInit {
  ...
  driverStatus: any;
  provisionalRemakrs: any;

  enableHimanchalBonofide = false;
  enableTenPlus  = false;
  gorkha = false;
  hMVLicense = false;
  category  = false;
  ...
  initializeForm() {
    ...
    this.manageAdmitCardsForm = new FormGroup({
      provisionalRemakrs: new FormControl(''),
    ...
    this.checkListForm = new FormGroup({
      bonafied: new FormControl('', Validators.required),
      tenPlus: new FormControl('', Validators.required),
      isGorkha: new FormControl(''),
      lic: new FormControl(''),
      cat: new FormControl(''),
  ...
  getApplicationSubmitted(application) {
    this.gorkha = false;
    this.hMVLicense = false;
    this.category = false;
    this.categoryDesc = undefined;
    this.subCategory = false;
    this.subCategoryDesc = undefined;
    this.enableScanner = false;
    this.applicationService.getApplicationScrutiny(application, 1).then((resp: any) => {
      if (resp.status == 200) {
        if (resp['body']['responseCode'] == 200) {
          console.log('Resp', resp['body']['responseObject']);
          this.response = resp['body']['responseObject'];
          
          // this.enableHimanchalBonofide = true;
          // this.enableTenPlus = true;
          ...
          this.manageAdmitCardsForm.controls['provisionalRemakrs'].setValue(response.provisionalRemakrs);
          this.provisionalRemakrs = response.provisionalRemakrs
          ...
          if(response.isGorkha == 'Yes') {
            this.gorkha = true;
            this.checkListForm.controls['isGorkha'].setValidators(Validators.required)
            this.checkListForm.controls['isGorkha'].updateValueAndValidity()
          } else {
            this.checkListForm.controls['isGorkha'].clearValidators();
            this.checkListForm.controls['isGorkha'].updateValueAndValidity()
          }
         if(postDesc == 'Driver') {
          this.hMVLicense = true;
          this.checkListForm.controls['lic'].setValidators(Validators.required)
          this.checkListForm.controls['lic'].updateValueAndValidity()
         } else {
          this.checkListForm.controls['lic'].clearValidators();
          this.checkListForm.controls['lic'].updateValueAndValidity()
        }

         console.log('this.response.category' , this.response.category)
         console.log('this.response.category' , this.response.category)
         console.log('this.response.category' , this.response.category)
         console.log('this.response.category' , this.response.category)
         console.log('this.response.category' , this.response.category)
          if(response.category != 'General') {
            this.category = true;
            this.categoryDesc = response.category;
            this.checkListForm.controls['cat'].setValidators(Validators.required)
            this.checkListForm.controls['cat'].updateValueAndValidity()
          } else {
            this.checkListForm.controls['cat'].clearValidators();
            this.checkListForm.controls['cat'].updateValueAndValidity()
          }
          
          if(response.subCategory != 'Others') {
            this.subCategory = true;
            this.subCategoryDesc = response.subCategory;
            this.checkListForm.controls['subCat'].setValidators(Validators.required)
            this.checkListForm.controls['subCat'].updateValueAndValidity()
          } else {
            this.checkListForm.controls['subCat'].clearValidators();
            this.checkListForm.controls['subCat'].updateValueAndValidity()
          }
          
          // this.checkListForm = new FormGroup({
          //   bonafied: new FormControl(''),
          //   tenPlus: new FormControl(''),
          //   : new FormControl(''),
          //   lic: new FormControl(''),
  ...
  updateHeightConfirmation(x) {
    if(x == 0) {
      this.openmd(this.secondConsentPopUp, x)
    } else {
      this.openmd(this.consentPopUp, x)
    }
  ...
  openmd(content, x) {
    this.modalService.open(content, {
      ...
      if (result == 'first consent') {
        ...
        this.doubleConfirm(x);
      } else if (result == 'second consent') {
        this.submitApplication(x);
  ...
  doubleConfirm(x) {
    this.openmd(this.confFormPopUP, x);
  }
  ...
  submitApplication(x) {
    ...
    let subCat;
    let dob;
    if(this.checkListForm.controls['bonafied'].value) {
      bonafied = 'Bonafide Himachali' + '|'
    } else {
      bonafied = null + '|'
    }
    if(this.checkListForm.controls['tenPlus'].value) {
      tenPlus = '10+2'+ '|'
    } else {
      tenPlus = null
    }
    if(this.checkListForm.controls['isGorkha'].value) {
      isGorkha = 'Gorkha'+ '|'
    } else {
      isGorkha = null + '|'
    }
    if(this.checkListForm.controls['lic'].value) {
      lic = 'HMV License'+ '|'
    } else {
      lic = null + '|'
    }
    if(this.checkListForm.controls['cat'].value) {
      cat = this.categoryDesc + '|'
    } else {
      cat = null + '|'
    }
    if(this.checkListForm.controls['subCat'].value) {
      subCat = this.subCategoryDesc + '|'
    }  else {
      subCat = null + '|'
    }

    if(this.checkListForm.controls['dob'].value) {
      dob = 'DOB' + '|'
    }  else {
      dob = null + '|'
    }
    ...
    if(x == 0) {
      reqObj = {
        ...
        registrationRemarks: this.remarksForm.controls['remarks'].value ,
        status:22,
        mode: 'SCR',
        registrationCheckList: bonafied + tenPlus + isGorkha+ lic + cat + subCat + dob
      }
    } else if(x == 1) {
      reqObj = {
        action: 1,
    ...
    // if(this.bonafied && this.tenPlus && (this.gorkha)) {
    ...
    let error 
    console.log('checkListForm ' , this.checkListForm.valid)
    for (let el in this.checkListForm.controls) {
      if (this.checkListForm.controls[el].errors) {
        console.log(el)
      }
 }  
    console.log('checkListForm error ' , this.checkListForm.errors)
    // let isFormValid = () :boolean => Object.values(this.checkListForm.controls).filter(c => c.invalid).length === 0
    // console.log('checkListForm' , isFormValid)
    if(this.checkListForm.valid) {
      // reqObj.registrationCheckList = ''
      ...
      this.sharedService.showError('Please complete the form')
    }
    
  }
  
  downloadAdmitCard() {
    let applicationNo =  this.searchForm.controls['applicationNumber'].value || this.qrResultString
    this.jsService.scrutinyAdmitCardPdf(applicationNo).then((resp: any) => {
      if (resp) {
        // this.blob = new Blob([resp], { type: 'application/pdf' });
        const downloadURL = window.URL.createObjectURL(resp);
        const link = document.createElement('a');
        link.href = downloadURL;
        link.download = "Admit card-Scrutiny-"+applicationNo +".pdf";
        link.click();
  ...
  checkValueBonafied(event) {
    if(event.currentTarget.checked == true) {
      this.bonafied = true;
      this.checkListForm.controls['bonafied'].setValue(true)
    } else {
      this.checkListForm.controls['bonafied'].setValue(null)
      this.bonafied = null;
    }
   
    console.log('checkValueBonafied' , event.currentTarget.checked)
  }

  checkValueTestPlus(event) {
    if(event.currentTarget.checked == true) {
      this.tenPlus = true;
      this.checkListForm.controls['tenPlus'].setValue(true)
    } else {
      this.checkListForm.controls['tenPlus'].setValue(null)
      this.tenPlus = null;
    } 
    console.log('checkValueTestPlus' , event.currentTarget.checked)
  }

  checkDob(event) {
    if(event.currentTarget.checked == true) {
      this.tenPlus = true;
      this.checkListForm.controls['dob'].setValue(true)
    } else {
      this.checkListForm.controls['dob'].setValue(null)
      this.tenPlus = null;
    } 
  }

  checkValueGorkha(event) {
    if(event.currentTarget.checked == true) {
      this.isGorkha = true;
      this.checkListForm.controls['isGorkha'].setValue(true)
    } else {
      this.isGorkha = null;
      this.checkListForm.controls['isGorkha'].setValue(null)
    }
    this.isGorkha = event.currentTarget.checked;
    console.log('checkValueGorkha' , event.currentTarget.checked)
  }

  checkValueLic(event) {
    if(event.currentTarget.checked == true) {
      this.lic = true;
      this.checkListForm.controls['lic'].setValue(true)
    } else {
      this.checkListForm.controls['lic'].setValue(null)
      this.lic = null;
    }
    // this.lic = event.currentTarget.checked;
    console.log('checkValueLic' , event.currentTarget.checked)
  }

  checkValueCat(event) {
    if(event.currentTarget.checked == true) {
      this.cat = true;
      this.checkListForm.controls['cat'].setValue(true)
    } else {
      this.checkListForm.controls['cat'].setValue(null)
      this.cat = null;
    }
    // this.cat = event.currentTarget.checked;
    console.log('checkValueCat' , event.currentTarget.checked)
  }

  checkValueSubCat(event) {
    if(event.currentTarget.checked == true) {
      this.subCat = true;
      this.checkListForm.controls['subCat'].setValue(true)
    } else {
      this.checkListForm.controls['subCat'].setValue(null)
      this.subCat = null;
    }
    // this.subCat = event.currentTarget.checked;
    console.log('checkValueSubCat' , event.currentTarget.checked)
  }
}
