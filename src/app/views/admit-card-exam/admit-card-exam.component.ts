...
@Component({
  selector: 'app-admit-card-exam',
  templateUrl: './admit-card-exam.component.html',
  styleUrls: ['./admit-card-exam.component.css']
})
export class AdmitCardExamComponent implements OnInit {
  ...
  admitCardSummary() {
    this.sharedService.getLocalStorageItem('district').then((resp: any) => {
      if (resp) {
        this.applicationService.admitCardSummaryExam(resp.id).then((res: any) => {
          if (res.status == 200) {
            if (res['body']['responseCode'] == 200) {
  ...
  confirmation() {
    ...
    if (this.generateAdmitCardForm.valid) {
      ...
      //     this.petTime = hr + ':' + min + ':00';
          ...
          this.venueName = venueArray[0].name;
  ...
  generateAdmitCard() {
    this.sharedService.confirm2('Confirm', 'Do you want to schedule the written exam and generate the admit cards?').then((res: any) => {
    ...
    this.sharedService.getLocalStorageItem('district').then((district: any) => {
      if (district) {
        ...
        const reqObj = {
          ...
          venue: this.generateAdmitCardForm.controls['venue'].value,
          examDate: '2022-07-03',
          noOfCandidate: this.generateAdmitCardForm.controls['noCandidates'].value,
          pageSize: 0
         };
        console.log('Req Obj ', reqObj);
        this.applicationService.generateExamAdmitCard(reqObj).then((resp: any) => {
          if (resp.status == 200) {
  ...
  applicationDetails(x) {
    ...
    this.router.navigateByUrl('/ExamDetails');
  }
